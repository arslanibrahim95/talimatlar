...
export class ErrorHandler {
  ...
  private sendToSentry(error: AppError): void {
    try {
      ...
      import('./sentryIntegration').then(({ captureException }) => {
  ...
  private handleBySeverity(error: AppError): void {
    switch (error.severity) {
      case ErrorSeverity.CRITICAL:
        // Critical errors might require immediate user notification
        this.notifyUser(error, 'critical');
        break;
      case ErrorSeverity.HIGH:
        // High severity errors should be logged and potentially notified
  ...
  /**
   * Notifies user about errors (placeholder)
   */
  private notifyUser(error: AppError, level: 'critical' | 'warning'): void {
    // This could integrate with a notification system
    console.log(`User notification (${level}):`, error.message);
  }
